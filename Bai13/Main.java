    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < string.length(); i++) { //Vòng lặp bên ngoài
            ...
            if (list.size() > max.size()) {
                //Nếu kích cỡ của list trung gian lớn hơn kích cỡ của list có độ dài
                //lớn nhất thì gán lại max
                max.clear();
                max.addAll(list);
            }
            list.clear();
        ...
        for (Character character : max) { // 1 vòng lặp
            System.out.println(character); // 1 câu lệnh
    }
        System.out.println();
